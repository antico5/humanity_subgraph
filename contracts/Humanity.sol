...
contract Governance {
    ...
    uint256 public constant OPEN_VOTE_PERIOD = 2 days;
    uint256 public constant VETO_PERIOD = 2 days;
    uint256 public constant TOTAL_VOTE_PERIOD = OPEN_VOTE_PERIOD + VETO_PERIOD;
    ...
    constructor(IERC20 _token, uint256 _initialProposalFee) public {
        token = _token;
    ...
    function deposit(uint256 amount) public {
        require(
            ...
            "Governance::deposit: Transfer failed"
        );
    ...
    function proposeWithFeeRecipient(
        address feeRecipient,
        address target,
        bytes memory data
    ) public returns (uint256) {
        require(
            msg.sender != address(this) && target != address(token),
            "Governance::proposeWithFeeRecipient: Invalid proposal"
        );
        require(
            token.transferFrom(msg.sender, address(this), proposalFee),
            "Governance::proposeWithFeeRecipient: Transfer failed"
        );
    ...
    function voteYes(uint256 proposalId) public {
        ...
        require(
            time() <= proposal.startTime.add(OPEN_VOTE_PERIOD),
            "Governance::voteYes: Proposal is no longer accepting yes votes"
        );

        uint256 proposalEndTime = proposal.startTime.add(TOTAL_VOTE_PERIOD);
        if (proposalEndTime > withdrawTimes[msg.sender])
            withdrawTimes[msg.sender] = proposalEndTime;

        uint256 weight = deposits[msg.sender].sub(
            yesVotes[proposalId][msg.sender]
        );
    ...
    function voteNo(uint256 proposalId) public {
        ...
        uint256 _time = time();
        require(
            _time <= proposalEndTime,
            "Governance::voteNo: Proposal is no longer in voting period"
        );

        uint256 _deposit = deposits[msg.sender];
        uint256 weight = _deposit.sub(noVotes[proposalId][msg.sender]);
    ...
    function removeVote(uint256 proposalId) public {
        Proposal storage proposal = proposals[proposalId];
        require(
            proposal.result == Result.Pending,
            "Governance::removeVote: Proposal is already finalized"
        );
        require(
            time() <= proposal.startTime.add(TOTAL_VOTE_PERIOD),
            "Governance::removeVote: Proposal is no longer in voting period"
        );

        proposal.yesCount = proposal.yesCount.sub(
            yesVotes[proposalId][msg.sender]
        );
        proposal.noCount = proposal.noCount.sub(
            noVotes[proposalId][msg.sender]
        );
    ...
    function finalize(uint256 proposalId) public {
        ...
        if (proposal.yesCount > proposal.noCount) {
            ...
            require(
                _time > proposal.startTime.add(OPEN_VOTE_PERIOD),
                "Governance::finalize: Proposal cannot be terminated until end of yes vote period"
            );
...
contract HumanityGovernance is Governance {
    constructor(IERC20 humanity, uint256 proposalFee)
        public
